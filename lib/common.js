  ...
  switch (column.type) {
    ...
    case MysqlTypes.ENUM:
      ...
      if (raw > choices.length) {
        // This can happen after an enum has been added during an alter table.
        // Proper solution would be to have zongji detect ALTER TABLE events, and then have it refresh table info using _fetchTableInfo.
        // For now we throw an error and should fix the sqlshift.state file manually
        throw new Error("invalid enum value in zongji. Resetting table map info could fix this? Column:" + JSON.stringify(column));
      }
